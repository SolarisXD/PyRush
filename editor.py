...
class Editor:
	def __init__(self, land_tiles, switch, return_to_menu=None):
		...
		# music (volume will be set by main.py when editor is created)
		self.editor_music = pygame.mixer.Sound('audio/Explorer.ogg')
		self.editor_music.set_volume(0.4)  # Default, will be overridden immediately
	...
	def clear_grid(self):
		"""Clear all tiles and objects from the canvas, keeping only player and sky handle"""
		# Clear canvas data (all tiles)
		self.canvas_data.clear()
		
		# Remove all canvas objects except player (tile_id 0) and sky handle (tile_id 1)
		for obj in list(self.canvas_objects):
			if obj.tile_id not in [0, 1]:  # Keep player and sky
				obj.kill()
		
		# Reset clouds
		self.current_clouds.clear()
		
		print("Grid cleared! Player and sky handle preserved.")


	# Save/Load functionality
	def save_level_with_ui(self):
		"""Save level with slot selection UI"""
		save_ui = SaveSlotUI(self.save_manager, mode='save')
		selected_slot = save_ui.show(save_type='level')
		
		if selected_slot is not None:
			level_data = self.create_grid()
			level_name = f"Custom Level {selected_slot + 1}"
			success = self.save_manager.save_level(selected_slot, level_data, level_name)
			if success:
				self.last_save_slot = selected_slot
				print(f"Level saved to slot {selected_slot + 1}")
	
	def load_level_with_ui(self):
		"""Load level with slot selection UI"""
		save_ui = SaveSlotUI(self.save_manager, mode='load')
		selected_slot = save_ui.show(save_type='level')
		
		if selected_slot is not None:
			level_data = self.save_manager.load_level(selected_slot)
			if level_data:
				self.load_level_data(level_data)
				self.last_save_slot = selected_slot
				print(f"Level loaded from slot {selected_slot + 1}")
	
	def quick_save(self):
		"""Quick save to the last used slot (default slot 0)"""
		if not hasattr(self, 'last_save_slot'):
			self.last_save_slot = 0
		
		level_data = self.create_grid()
		level_name = f"Quick Save {self.last_save_slot + 1}"
		success = self.save_manager.save_level(self.last_save_slot, level_data, level_name)
		if success:
			print(f"Quick saved to slot {self.last_save_slot + 1}")
	
	def quick_load(self):
		"""Quick load from the last used slot (default slot 0)"""
		if not hasattr(self, 'last_save_slot'):
			self.last_save_slot = 0
		
		level_data = self.save_manager.load_level(self.last_save_slot)
		if level_data:
			self.load_level_data(level_data)
			print(f"Quick loaded from slot {self.last_save_slot + 1}")
	
	def load_level_data(self, level_data):
		"""Load level data into the editor"""
		# Clear existing canvas
		self.canvas_data.clear()
		for sprite in self.canvas_objects:
			if sprite.tile_id not in [0, 1]:  # Don't delete player and sky handle
				sprite.kill()
		
		# Reconstruct canvas from loaded data
		for layer_name, layer in level_data.items():
			for pos, data in layer.items():
				cell_pos = (pos[0] // TILE_SIZE, pos[1] // TILE_SIZE)
				
				# Handle terrain
				if layer_name == 'terrain':
					if cell_pos not in self.canvas_data:
						self.canvas_data[cell_pos] = CanvasTile(2)  # terrain tile ID
					else:
						self.canvas_data[cell_pos].add_id(2)
				
				# Handle water
				elif layer_name == 'water':
					if cell_pos not in self.canvas_data:
						self.canvas_data[cell_pos] = CanvasTile(3)  # water tile ID
					else:
						self.canvas_data[cell_pos].add_id(3)
				
				# Handle coins
				elif layer_name == 'coins':
					if cell_pos not in self.canvas_data:
						self.canvas_data[cell_pos] = CanvasTile(data)
					else:
						self.canvas_data[cell_pos].add_id(data)
				...
				# Handle bg palms and fg objects
				elif layer_name in ['bg palms', 'fg objects']:
					groups = [self.canvas_objects, self.background] if layer_name == 'bg palms' else [self.canvas_objects, self.foreground]
					CanvasObject(
						pos=pos,
						frames=self.animations[data]['frames'],
						tile_id=data,
						origin=self.origin,
						group=groups
					)
		
		# Update neighbors for all tiles
		for cell_pos in self.canvas_data.keys():
			self.check_neighbors(cell_pos)
	...
	def event_loop(self):
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				...
				elif event.key == pygame.K_F5:  # Quick save to last used slot
					self.quick_save()
				elif event.key == pygame.K_F9:  # Quick load from last used slot
					self.quick_load()
				elif pygame.key.get_pressed()[pygame.K_LCTRL] and event.key == pygame.K_s:  # Ctrl+S: Save with slot selection
					self.save_level_with_ui()
				elif pygame.key.get_pressed()[pygame.K_LCTRL] and event.key == pygame.K_l:  # Ctrl+L: Load with slot selection
					self.load_level_with_ui()
				elif pygame.key.get_pressed()[pygame.K_LCTRL] and event.key == pygame.K_n:  # Ctrl+N: Clear grid
					self.clear_grid()
	...
	def draw_help_overlay(self):
		...
		if not self.show_help:
			...
			hint_rect = hint_surf.get_rect(topleft=(10, 10))
		...
		# Title
		title = self.help_title_font.render("LEVEL EDITOR CONTROLS", True, (255, 215, 0))
		...
		controls = [
			"",
			"BUILDING:",
			"  Mouse Left Click - Place tile/object",
			"  Mouse Right Click - Delete tile/object",
			"  Arrow Keys ← → - Change selected tile",
			"  Click Menu (bottom-right) - Select tile category",
			"  Middle Mouse + Drag - Pan camera",
			"  Mouse Wheel - Scroll camera",
			"",
			"PLAYING:",
			"  ENTER - Play your level",
			"  ESC - Return to main menu",
			"",
			"SAVING:",
			"  F5 - Quick save to last slot",
			"  F9 - Quick load from last slot",
			"  Ctrl+S - Save with slot picker",
			"  Ctrl+L - Load with slot picker",
			"  DEL/D (in slot picker) - Delete save",
			"  Ctrl+N - Clear entire grid (New level)",
			"",
			"HELP:",
